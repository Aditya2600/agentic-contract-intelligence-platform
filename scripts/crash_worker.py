...
Started by `scripts/crash_demo.py` (`make demo-crash`) and by
`tests/test_crash_resume.py`. It has to be a separate process: SIGKILL skips `finally`
blocks, pool drain and any checkpoint flush, which is exactly the failure mode both the
demo and the test are trying to prove survivable. Reads a JSON spec file.

Not a standalone entry point -- it is always launched by one of those two, which supply
the spec and assert on what the database looks like afterwards.
"""
