...
- **Injection scan, at the block boundary:** every block is scanned the instant it becomes a block (`services/injection.py:scan`, before the row is even inserted) and its result is stamped onto that one row as `document_blocks.injection_flag`. This is deliberately per block, not per document: a document-level scan-and-quarantine makes one hostile paragraph a denial-of-service lever over the other fifty-nine legitimate ones in the same file. `scan()` first runs `normalise()` — strips Unicode tag characters and invisible/bidi-control characters (soft hyphen, zero-width space/joiner/non-joiner, LRM/RLM, bidi embedding/override, word joiner block, BOM), HTML-unescapes entities, NFKC-folds, and unwraps (never deletes) text hidden inside HTML comments/attributes and Markdown link-titles/alt-text — then matches seven named imperative categories (`override_instructions`, `role_impersonation`, `approval_demand`, `concealment`, `secret_exfiltration`, `tool_invocation`, `instruction_to_agent`) against the normalised text. Text read by a vision model (`extraction_method != "txt"`) that also matches an imperative gets an extra `read_by_<method>` signal, since the same imperative reaching the model through OCR is not something a human looking at the rendered page would ever have seen.
...
- **Purpose:** Surfaces what `parse_blocks` already found. The scan itself ran per block one stage earlier; this node reads `state["quarantined_blocks"]`, appends one `extraction_warnings` entry per quarantined block (`"block N withheld from extraction and rule context (<signals>)"`), and sets the run-level `injection_flag`.
- **Telemetry, never permission:** there is no branch here on a *negative* result. A clean scan does not relax anything downstream — every proposal still needs an explicit human decision (`force_review` is unconditionally `True` in every review-item payload, regardless of whether anything was flagged), the model is never handed a database connection, an approval capability, or a tool credential, and the register still cannot be written by document text. The security boundary is that architecture, not this scan; the scan only decides what a human is shown first.
- **Extraction warning, not `quarantined` status:** a withheld block is folded into the same `extraction_warnings` channel that `_rules_summary` already refuses to call `clean` over (a rotated page, an unreadable scan, and a withheld injection block are all "evidence the pipeline could not fully use," and `clean: true` is unreachable while any of them stand). No document-level `status = 'quarantined'` is ever set — `documents.status` is legacy-read-only for that value (`repositories/postgres.py`); withholding is a per-block property (`document_blocks.injection_flag`), not a per-document one.
...
- **Flagged blocks are never sent to the model:** `extract_facts` skips `deps.model.extract()` entirely for any block with `injection_flag = True` — not filtered afterward, never called. This is both the containment (a block that tried to give instructions does not get to propose a value) and the only way to guarantee its text never reaches a prompt. The blocks around it in the same document are still sent normally.
...
- **Context Bounding:** Uses `select_excerpts` to rank document blocks by keyword overlap with rule text, capping input to `DOCTASK_RULE_CONTEXT_BLOCKS` (12) and `DOCTASK_RULE_CONTEXT_CHARS` (8000). `select_excerpts` filters out `injection_flag = True` blocks before scoring — a rule prompt is a second door into the same model, and withholding a block from extraction while still handing it over here as "context" would be the same failure with an extra step. A rule left with nothing to read reaches `insufficient_evidence`, which is the honest answer.
...
| **Injection Containment** | Per-block scan (`services/injection.py`) is telemetry, never permission — a clean result grants nothing. A flagged block is withheld from extraction (`extract_facts` never calls the model on it), from rule context (`select_excerpts`, `_register_excerpts`), and from the register, while the rest of the document is processed normally. `force_review` is unconditional; the model is handed structured block data only, never a database, approval, or tool credential. | `parse_blocks` (scan), `extract_facts`, `select_excerpts`, `assemble_proposals` (`injection_review` item) |
